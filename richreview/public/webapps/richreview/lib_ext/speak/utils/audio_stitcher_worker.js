this.onmessage = function(e){
    ...
    if ( e.data.state === 'start' ) {
        var src_wavs = {};
        var wav_urls = e.data.urls;
        var snippets = e.data.snippets;
        var final_wav = {};
        var src_streams = {};
            ...
            if(format_key !== 'stream' && format_key !== 'byte_array'){ // stream is not a format parameter
                if(!wav.hasOwnProperty(format_key)){
                    ...
                    if(wav[format_key] !== wavs[key][format_key]){
                        console.log(wav[format_key], wavs[key][format_key]);
...
function done(data){
  ...
  this.postMessage({state:'done', view:data});
}
