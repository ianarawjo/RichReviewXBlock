...
(function(r2) {
    ...
    r2.speak = (function() {
        ...
        class Talken {
            ...
            static generate(timestamps, audioURL) {
                ...
                for (let t of timestamps) {
                    ...
                    if (prev_t) {
                        if ((t[1] === 0 && t[2] === 0) || (prev_t[1] === 0 && prev_t[2] === 0)) {}
                        else {
                            var pause_len_ms = Math.round((t[1] - prev_t[2]) * 1000.0); // current bgn - previous end
                            if (pause_len_ms > PAUSE_THRESHOLD_MS) { // if pause length is significant...
            ...
            static generateFromHTK(audioURL, perfect_transcript) {
            ...
            static clone(t) {
                if (t instanceof Talken) return t.clone();
                else if (t instanceof Array) return t.map((tn) => tn.clone());
                else {
            ...
            return {
                ...
                stitch : function(talkens) { // Returns stitched audio resource as a Promise.
                    return r2.audiosynth.stitch(talkens).then(function(sobj) {
                        return new Promise(function(resolve, reject) {
                            if (!sobj.url) reject("Audio.stitch: Null url.");
                            else resolve(sobj.url);
                        });
                    });
                },
                ...
                patch : function(talkens) {
                    return r2.audiosynth.patch(talkens).then(function(sobj) {
                        return new Promise(function(resolve, reject) {
                            if (!sobj.url) reject("Audio.patch: Null url.");
                            else resolve(sobj.url);
                        });
                    });
                },
        ...
        pub_speak.controller = () => {
             ...
             */
            pub.insertVoice = (index, ts, annotId) => {
            ...
            pub.flatten = () => { // Turn edited into base.
                if (!edited || !base) {
            ...
            pub.update = (new_transcript) => {
                ...
                for (let w = 0; w < wrds.length; w++) { // isolate pause markers from attachments to any words
                    ...
                    if (space_char_i > -1) {
                        ...
                        if (space_char_i < wrds[w].length / 2)
                            wrds.splice(w, 0, '♦');
                        else
                            wrds.splice(w+1, 0, '♦');
                        w++;
            ...
            var _render = (mode, options) => {
                ...
                // Compile the audio
                var after_stitching = function(stitched_url) {
    ...
    r2.audiosynth = (function() {
        ...
        var toTimestamps = (talkens) => {
            ...
            talkens.forEach((t) => {
                if (typeof t.audio === "undefined" || !t.audio || !t.audio.url)
                    ts.push([ t.word.replace(',',''), 0, 0 ]);
                else
                    ts.push([ t.word.replace(',',''), t.bgn, t.end ]);
            });
            return ts;
        };
         ...
         */
        pub.smoothMissingTimestamps = (talkens) => {
            var ratio, len;
            var nextTalken = function(idx, tks) {
            ...
            var smooth = function(brokenTk, targetTk, isNext) {
                ...
                } else { // target talken is before the broken one
        ...
        var toSSML = (talkens) => {
            ...
            for (var i = 1; i < words.length; i++) {
                ...
                if (ts.pauseBefore > 0) breaks.push(Math.min(ts.pauseBefore, PAUSE_MAX_MS));
                else if (prev_ts.pauseAfter > 0) breaks.push(Math.min(prev_ts.pauseAfter, PAUSE_MAX_MS));
                else breaks.push(0);
        ...
        var getTTSAudioURL = (ssml, voice="en-US_MichaelVoice") => {
        ...
        pub.synthesize = (talkens, config) => {
            ...
            return stitch(talkens).then(function(sobj) {
                ...
                else if (target_ts.length !== orig_src_ts.length) throw 'Timestamp data mismatch: ' + src_ts.length + ' != ' + target_ts.length;
